...
def post_doctor():
    ...
    if 'doctor' not in session and 'patient' not in session:
        if request.method == 'POST':
            ...
            doctor = Doctor(d_name=name, d_education=education, d_registerationno=regno, d_aadharno=aadharno,
                            d_mobileno=mobno, d_mailid=email, d_address=add, d_password=pwd)
            Session.add(doctor)
            Session.commit()
            return redirect(url_for('index'))
...
def post_patient():
    ...
    if 'doctor' not in session and 'patient' not in session:
        if request.method == 'POST':
            ...
            add = request.form['address']
            pwd = request.form['password']
            patient = Patient(p_name=name, p_birthdate=dob, p_sex=sex, p_weight=weight, p_bloodgroup=bloodgroup,
                              p_aadharno=aadharno, p_mobileno=mobileno, p_mailid=email, p_address=add, p_password=pwd)
...
def doctor_patient_add_report():
    ...
    if 'doctorpatient' in session:
        ...
        reports = Session.query(Report).filter_by(r_pid=patient.p_id, visible=1).all()
        if request.method == 'POST':
            report = Report(date=request.form['date'], patient_name=patient.p_name, symptoms=request.form['symptoms'],
                            diagnosis=request.form['diagnosis'], treatment=request.form['treatment'],
                            doctor_name=doctor.d_name, patient=patient, doctor=doctor)
            Session.add(report)
            Session.commit()
    ...
    if 'doctorpatient' in session:
        ...
        if request.method == 'POST':
            ...
            for upload in request.files.getlist("file"):
                ...
                newScan = Scans(description=request.form['description'], paths=filename, s_date=request.form['date'],
                                patient=patient)
                Session.add(newScan)
                Session.commit()
